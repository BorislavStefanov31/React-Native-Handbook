# React Native Handbook
 
A comprehensive React Native handbook covering **54 in-depth topics** — from fundamentals to production.
 
📖 [Download the PDF](React_Native_Handbook.pdf)
 
## What's Inside
 
- **Fundamentals** — Threading, JSI, Hermes engine
- **Architecture** — Offline sync, state management, navigation patterns
- **Production** — CI/CD, deployment, monitoring, security
 
## About
 
The questions are based on real-world problems and topics I've encountered in my day-to-day work and things I genuinely wanted to understand better. The answers were researched using AI (Claude Opus 4.6 Deep Research), synthesizing best practices, approaches, and architectural decisions described by engineers from leading React Native teams — Meta, Callstack, Software Mansion, and other top companies in the ecosystem.
 
I personally learned a lot putting this together. Open to suggestions if you think something can be added or improved.
 
## License
 
Feel free to use this as a learning resource. Contributions and feedback are welcome — open an issue or submit a PR.
